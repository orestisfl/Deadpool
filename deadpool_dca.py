...
class DefaultFilters:
    ...
    stack_w1 = Filter(
        "stack_w1",
        ["W"],
        lambda stack_range, addr, size, data: stack_range[0] <= addr <= stack_range[1]
        and size == 1,
    ...
    mem_addr1_rw1 = Filter(
        ...
        lambda stack_range, addr, size, data: (
            addr < stack_range[0] or addr > stack_range[1]
        )
        and size == 1,
...
def findbin(keyword):
    ...
    for filename in glob.glob("trace_%s_*.bin" % keyword):
        i, iblock, oblock = filename[len("trace_%s_" % keyword) : -len(".bin")].split(
            "_"
        )
        if iblock != "na":
...
def bin2daredevil(
    keyword=None, keywords=None, delete_bin=True, config=None, configs=None
):
    ...
    for keyword in keywords:
        ...
        with open(
            "%s_%i_%i.trace" % (keyword, ntraces, nsamples), "wb"
        ) as filetrace, open(
            "%s_%i_%i.input" % (keyword, ntraces, nsamples), "wb"
        ) as fileinput, open(
            "%s_%i_%i.output" % (keyword, ntraces, nsamples), "wb"
        ) as fileoutput:
...
""" % (
                nsamples,
                "%s_%i_%i.trace" % (keyword, ntraces, nsamples),
                ntraces,
                nsamples,
                "%s_%i_%i.%s" % (keyword, ntraces, nsamples, config["guess"]),
                ntraces,
                blocksize,
                config["threads"],
                config["algorithm"],
    ...
    for keyword in keywords:
        ...
        with open("%s_%i_%i.trs" % (keyword, ntraces, nsamples), "wb") as trs:
            ...
            trs.write(
                "\x44\x02"
                + struct.pack(
                    "<H", blocksize * iblock_available + blocksize * oblock_available
                )
            )
            # End of header
            trs.write("\x5F\x00")
            ...
            for i, line in enumerate(info):
                if i + 1 == ievent:
                    ...
                    try:
                        ...
                        output = ""
                    eventlist.append(
                        (mem_mode, item, ins_addr, mem_addr, mem_size, mem_data, output)
                    )
                elif i > ievent:
                    break
...
class Tracer(object):
    def __init__(
        ...
        if self.stack_range != "default":
            self.stack_range = (
                int(stack_range[: stack_range.index("-")], 16),
                int(stack_range[stack_range.index("-") + 1 :], 16),
            )
        if filters == "default":
    ...
    def _trace_dump(self):
        ...
        for f in self.filters:
            ...
            if f.record_info:
                with open(
                    "trace_%s_%s_%s.info" % (f.keyword, iblockstr, oblockstr), "wb"
                ) as trace:
                    for (
                        mem_mode,
                        item,
...
class TracerPIN(Tracer):
        ...
        with open(self.tmptracefile, "r") as trace:
            for line in iter(trace.readline, ""):
                if len(line) > 2 and (line[1] == "R" or line[1] == "W"):
                    ...
                    for f in self.filters:
                        if mem_mode in f.modes and f.condition(
                            self.stack_range, mem_addr, mem_size, mem_data
                        ):
                            if f.record_info:
                            ...
                            self._trace_data[f.keyword].append(
                                f.extract(mem_addr, mem_size, mem_data)
                            )
        self._trace_dump()
        if not self.debug:
    ...
    def run_once(self, iblock=None, tracefile=None):
        ...
        if input_args is None:
            input_args = []
        cmd_list = (
            [tracerpin_exec, "-f", str(self.addr_range), "-o", tracefile, "--"]
            + self.target
            + input_args
        )
...
class TracerGrind(Tracer):
    def __init__(
        ...
        super(TracerGrind, self).__init__(
            target,
            processinput,
            processoutput,
            arch,
    ...
    def get_trace(self, n, iblock):
        ...
        cmd_list = (
            [
                ...
                "--trace-children=yes",
                "--tool=tracergrind",
                "--filter=" + str(self.addr_range),
                "--vex-iropt-register-updates=allregs-at-mem-access",
                "--output=" + self.tmptracefile + ".grind",
            ]
            + self.target
            + input_args
        )
        ...
        with open(self.tmptracefile, "r") as trace:
            for line in iter(trace.readline, ""):
                ...
                mem_addr = int(
                    line[
                        line.index("START_ADDRESS")
                        + 15 : line.index("START_ADDRESS")
                        + 31
                    ],
                    16,
                )
                mem_size = int(
                    line[line.index("LENGTH") + 7 : line.index("LENGTH") + 10]
                )
